...
<body>
	...
	<form action="sys/sys_edit_periode.php?id=<?= $_GET['id'] ?>" method="POST" enctype="multipart/form-data">
	<div class="col-md-9" style="margin-bottom: 16px;">
		<div class="page-backup-1">
			<p class="all-info"><i class="far fa-edit"></i>&nbsp;Edit Kepemimpinan  :</p>
			<input type="text" class="form-control title-here" name="nama" placeholder="Nama" value="<?= $_SESSION['edt_nama'] ?>">
			<br>
			<p class="all-info"><i class="far fa-clock"></i>&nbsp;Edit Periode Kepemimpinan :</p>
			<input type="text" class="form-control title-here" name="periode" placeholder="Periode"
			value="<?= $_SESSION['edt_periode'] ?>">
		</div>
	</div>
