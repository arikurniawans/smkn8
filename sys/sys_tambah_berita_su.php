<?php 
	...
	if (isset($_POST['upload'])) {
		if (empty($_POST['judul']) OR empty($_POST['isi'])) {
			...
			header('location:../tambah-info-su?pesan=Lengkapi_Berita');
		}else {
			...
			if ($content->error == 4) {
				...
				header('location:../tambah-info-su?pesan=Tambahkan_Foto_Berita');
				...
				if (move_uploaded_file($content->tmp_foto, $dir)) {
					...
					$q_tambah = "INSERT INTO berita VALUES('','$content->id_kategori','$content->id_user','$content->judul','$content->isi','$content->nama_foto','$content->tanggal','0')";
